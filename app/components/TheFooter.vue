...
const googlefcReady = ref(false)

onMounted(() => {
  const check = () => {
    if ((window as any).googlefc) {
      googlefcReady.value = true
    } else {
...
const manageCookies = () => {
  (window as any).googlefc?.callCommand('SHOW_REVOKE_AD_PERSONALIZATION_DIALOG')
}
</script>
...
<template>
  <footer class="footer p-4 bg-base-200 text-base-content border-t border-base-content/5 mt-2 flex flex-wrap items-center justify-between gap-y-4 px-6 md:px-10">
    ...
    <nav class="flex flex-wrap gap-x-6 gap-y-2 justify-center" aria-label="Footer Navigation">
      ...
      <button v-show="googlefcReady" class="link link-hover font-medium underline-offset-4" @click="manageCookies">{{ t('c') }}</button>
    </nav>
  </footer>
</template>
